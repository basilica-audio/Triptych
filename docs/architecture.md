...
`BandCompressor` optionally runs an additional `juce::dsp::Limiter<float>` stage after its compressor + makeup gain (`setLimiterEnabled()`/`setLimiterThresholdDb()`); the type is generic (any band could opt in) but only the High band exposes it via APVTS (`ParamIDs::highLimiterEnabled`, `ParamIDs::highLimiterThreshold`) - a fast safety ceiling for cymbal/harmonic transients that would otherwise need retuning the band's own compressor. `juce::dsp::Limiter` (JUCE 8.0.14, `juce_dsp/widgets/juce_Limiter.h`) is internally two cascaded `juce::dsp::Compressor`s plus a hard clip at exactly &plusmn;1.0 (0 dBFS) - the hard clip is unconditional, independent of the user threshold, which is what `tests/MuteSoloAndLimiterTests.cpp`'s "output never exceeds 0 dBFS" test relies on. Its internal makeup-gain compensation (`outputVolume`) actually *increases* as the threshold is pulled down (a loudness-maximiser design, not a simple peak-catcher), so output RMS is **not** monotonic with threshold - only the final hard clip is a strict guarantee, and the test suite is written around that rather than an incorrect monotonicity assumption. Toggling the stage on/off goes through `context.isBypassed` (`juce::dsp::Limiter::process`, JUCE 8.0.14) rather than an `if (limiterEnabled)` branch around the call in `BandCompressor::process()` - a cheap, real-time-safe copy-through with no allocation or branch on the hot path either way. Note this does **not** keep the limiter's internal ballistics continuous while disabled: `Limiter::process`'s `isBypassed` path is an early return (`outputBlock.copyFrom (inputBlock); return;`) *before* either internal `Compressor` or `outputVolume` is touched, so both stages' envelope state is simply frozen (not updated) for the duration the limiter is disabled, then resumes from wherever it was left when re-enabled - there is no guaranteed pop, since the limiter's own two-stage ballistics have no lookahead/attack transient large enough to produce one on resume, but "continuous ballistics" is not what actually prevents it. No lookahead/delay line anywhere in `juce::dsp::Limiter`, so this adds zero latency regardless of `isBypassed` state.

## Per-band Mute/Solo

Applied in `TriptychEngine::process()`'s summing loop, *after* each band's `BandCompressor` (including the High band's optional limiter) has already run (`ParamIDs::lowMute`/`lowSolo`, `midMute`/`midSolo`, `highMute`/`highSolo`). Console-style semantics: **Mute always wins** (a muted-and-soloed band is still silent); if **any** band is soloed, only soloed (and unmuted) bands reach the sum. The resolved per-band on/off decision drives a target of `1.0f`/`0.0f` on a dedicated `juce::SmoothedValue` per band (`lowMuteSoloGain`/`midMuteSoloGain`/`highMuteSoloGain`, linear, `smoothingTimeSeconds`), applied sample-accurately in the summing loop rather than as an instantaneous step - toggling Mute or Solo mid-playback ramps rather than clicks, the same reasoning that already applies to every other audible-level parameter in this engine (see [Parameter smoothing](#parameter-smoothing) below). Each band's own compressor/limiter keeps processing unconditionally regardless of Mute/Solo state, so envelope followers stay continuous too, independent of the summing-stage ramp. All six parameters default to off, so their addition never changes v0.1's original default behaviour. Because an LR4 crossover's -24 dB/octave rolloff means a muted/non-soloed band's own filter path still leaks a measurable (if strongly attenuated, typically 30+ dB down) amount of an in-band signal into neighbouring bands, `tests/MuteSoloAndLimiterTests.cpp` verifies Mute/Solo via a large relative level drop against an unmuted/unsoloed reference rather than asserting literal digital silence; a dedicated test also confirms the toggle ramps (bounded sample-to-sample delta) rather than stepping.
...
- **Mute/Solo** (per band) resolve to a `1.0f`/`0.0f` target that is itself ramped sample-accurately via a dedicated `juce::SmoothedValue` per band (`lowMuteSoloGain`/`midMuteSoloGain`/`highMuteSoloGain`, linear, `smoothingTimeSeconds`) in `TriptychEngine::process()`'s summing loop - not applied as an instantaneous step, so a toggle mid-playback never produces an audible click (see [Per-band Mute/Solo](#per-band-mutesolo) above).
- All smoothers are seeded to their real starting value in `prepare()` (`lastLowMidSplitHz`/`lastMidHighSplitHz` in `TriptychEngine`, `lastThresholdDb`/`lastRatio` in `BandCompressor`, the current Mute/Solo bools for the mute/solo gains), so re-preparing (sample-rate change, etc.) never resets a live parameter back to a built-in default or lets a smoother ramp from an invalid starting point (e.g. 0 Hz, or a ratio below the `juce::dsp::Compressor::setRatio` contract of `>= 1.0`) or introduces a spurious fade on top of a Mute/Solo state the user already committed to.
